...
lazy_static! {
    static ref DOWNLOAD_CHAN: Mutex<Option<AppHandle>> = Mutex::new(None);
}
...
#[tauri::command]
async fn add_download(download: Download) -> Response<String> {
    match add_download_file(download).await {
...
#[tauri::command]
async fn start_downloading_file(id: i32) -> Response<String> {
    match start_downloading(id).await {
        Ok(data) => create_res_ok("ok".to_string()),
...
#[tauri::command]
fn download_progress(app: AppHandle) {
    let mut handler = DOWNLOAD_CHAN.lock().unwrap();
    *handler = Some(app);
}
